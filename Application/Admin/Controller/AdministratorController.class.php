...
class AdministratorController extends MyController {
	...
	*/
	...
	public function _initialize()
	{
		...
		$this->searchPostUrl = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/'.ACTION_NAME);
        ...
        */
        ...
        $this->navList = D('Admin/SystemData')->get('admin_nav:top');
	...
	*/
	...
	public function displayConfig($detailData = false){
		...
		$this->display(dirname(__FILE__).'/../View/Administrator/admin_config.html');
	}
	...
	public function displayList($listData=array()){
		...
		if(!empty($this->searchKey)){
			...
			//$searchKeyData = D('SystemConfig')->pagekey_get('searchPageKey:'.$this->searchPageKey);
			$searchKeyData = D('Admin/SystemConfig')->pagekey_get('searchPageKey:'.$this->searchPageKey);
			//使用D()获取的searchKeyData 当为空的时候不为null，放在页面显示时会出现问题。所以当其为empty时，设置其为null
		...
		//define('THEME_PATH', './Application/Admin/View/');
		$this->display(dirname(__FILE__).'/../View/Administrator/admin_list.html');
	}
	...
	public function displayCateTree($tree = array()){
		...
		//页面Key配置保存的值
		//$pageKeyData = ('Xconfig')->pagekey_get('pageKey:'.$this->pageKey);
		
		$pageKeyData = D('Admin/SystemConfig')->pagekey_get('pageKey:'.$this->pageKey);
		...
		$this->display(dirname(__FILE__).'/../View/Administrator/admin_catetree.html');
	}
	...
	public function displayTree($tree = array(), $stable = null, $level = 0, $delParam = null, $extra = '', $limit = 0){
		...
		$this->display(dirname(__FILE__).'/../View/Administrator/admin_tree.html');
	}
	...
	private function _assignPageKeyData($detailData = false){
		...
		//使用D获取的pageKeyData 当为空的时候不为null，放在页面显示时会出现问题。所以当其为empty时，设置其为null
		
		if(empty($pageKeyData)){
			unset($pageKeyData);
		}								 
		$this->assign('pageKeyData',$pageKeyData);
		...
		if($detailData === false){
			...
			$detailData = D('Admin/SystemData')->get($this->systemdata_list.":".$this->systemdata_key);
		}
	...
	public function savePageConfig(){
		...
		if(D('Admin/SystemConfig')->pageKey_lput('pageKey',$data)){
	...
	public function saveSearchConfig() {
		...
		if ( D('Admin/SystemConfig')->pagekey_lput('searchPageKey', $data)){
			/*
			LogRecord ( 'admin_config', 'editSearchPagekey', array (
					'name' => $title,
					'k1' => L ( 'PUBLIC_ADMIN_EDIT_PEIZHI' ) 
			), true );
			*/
			$this->success ();
		} else {
	...
	public function saveConfigData() {
		...
		LogRecord ( 'admin_config', 'editDetail', array (
				'name' => $title,
				'k1' => L ( 'PUBLIC_ADMIN_EDIT_EDTAIL_PEIZHI' ) 
		), true ); // 保存修改编辑详细数据
	...
	public function permissionset() {
		...
		$this->assign ( 'moduleHash', array (
				'normal' => L ( 'PUBLIC_SYSTEM_NORMAL_USER' ),
				'admin' => L ( 'PUBLIC_SYSTEM_ADMIN_USER' ) 
		) );
	...
	*/
	public function display($templateFile = '', $charset = 'utf-8', $contentType = 'text/html') {
		...
		$this->assign ( 'savePageConfigUrl', $this->savePageConfigUrl );
	...
	private function _switchTheme($themeName = '') {
		...
		if ($pos === false) {
			$content = str_replace ( 'return array(', 'return array(\'THEME_NAME\' => \'' . $themeName . '\', ', $content );
		} else {
