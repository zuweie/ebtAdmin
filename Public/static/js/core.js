...
var _core = function() {
	...
	this._loadCss = function() {
		...
		var tempMethod = function(url) {
			...
			if(flag == 0) {
				// 未载入过
				temp[temp.length] = url;	
				var css = '<link href="'+PUBLIC_STATIC+'/js/tbox/box.css" rel="stylesheet" type="text/css">';
				$('head').append(css);
			}
		};
		// 返回内部包函数,供外部调用并可以更改temp的值
	...
	this._rcalendar = function(text, mode, refunc) {
		...
		var tempMethod = function(t, m, r) {
			...
			if(temp == 0) {	
				// JQuery的ajax载入文件方式，如果有样式文件，同理在此引入相关样式文件
				$.getScript(PUBLIC_STATIC+'/js/rcalendar.js', function() {	
					rcalendar(t, m, r);
				});
			} else {
				rcalendar(t, m, r);
		...
		var _c = function() {
			if(_imgHtml == '') {
				$.post(U('public/Feed/getSmile'), {}, function(data) {
					for(var k in data) {
						_imgHtml += '<a href="javascript:void(0)" title="'+data[k].title+'" onclick="core.face.face_chose(this)";><img src="'+ PUBLIC_STATIC +'/image/expression/'+data[k].type+'/'+ data[k].filename +'" width="24" height="24" /></a>';	
					}
...
core.plugInit = function() {
	if(arguments.length > 0) {
		...
		var file = PUBLIC_STATIC + '/js/plugins/core.' + arguments[0] + '.js';
		core.loadFile(file, back);
	}
};
//与上面方法类似 只不过可以自己写回调函数（不主动执行init）
core.plugFunc = function(plugName,callback){
	var file = PUBLIC_STATIC+'/js/plugins/core.'+plugName+'.js';
...
var ui = {
	...
	box: {
		...
		init: function(title, callback) {
			...
			var url = PUBLIC_STATIC+'/js/tbox/box.css';
			core.loadCss(url);
			// 添加头部
